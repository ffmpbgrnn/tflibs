...
class GRUDecCell(RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
      with vs.variable_scope("Gates"):  # Reset gate and update gate.
        # We start with bias of 1.0 to not reset and not update.
        r, u = array_ops.split(1, 2, _linear([inputs, state, self._h],
                                             2 * self._num_units, True, 1.0))
        r, u = sigmoid(r), sigmoid(u)
      with vs.variable_scope("Candidate"):
        c = self._activation(_linear([inputs, r * state, self._h],
                                     self._num_units, True))
      new_h = u * state + (1 - u) * c
    return new_h, new_h
          ...
          if self._state_is_tuple:
            ...
            if type(state) == list:
              cur_state = array_ops.slice(
                  state[-1], [0, cur_state_pos], [-1, cell.state_size])
              cur_state = [state[0], cur_state]
            else:
              cur_state = array_ops.slice(
...
def _linear(args, output_size, bias, bias_start=0.0, scope=None, weights_regularizer=None):
  ...
  with vs.variable_scope(scope or "Linear"):
    matrix = vs.get_variable(
        ...
        regularizer=weights_regularizer, dtype=dtype)
    if len(args) == 1:
