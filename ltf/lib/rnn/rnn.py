...
def rnn(cell, inputs, initial_state=None, dtype=None,
        sequence_length=None, scope=None, use_step=False):
  ...
  with vs.variable_scope(scope or "RNN") as varscope:
    ...
    prev_states = []
    state_size = tf.pack([-1, state.get_shape()[1]])
    for time, input_ in enumerate(inputs):
      prev_states.append(state)
      if time > 0: varscope.reuse_variables()
      # pylint: disable=cell-var-from-loop
      if use_step:
        call_cell = lambda: cell(input_, [time+1, state])
      else:
      ...
      if sequence_length is not None:
        (output, new_state) = _rnn_step(
            ...
            state=state,
            call_cell=call_cell,
            state_size=cell.state_size, use_step=use_step)
      else:
        (output, new_state) = call_cell()
        if use_step:
          new_state = new_state[-1]
      if False and FLAGS.phase_train and not FLAGS.do_cls:
        bernoulli = tf.random_uniform([1], 0, 1)[0]
        def use_new_state():
          outputs.append(output)
          return new_state
        state = tf.cond(bernoulli < 0.5, lambda: prev_states[time], use_new_state)
      else:
        state = new_state
        outputs.append(output)
      ...
      if time % 3 == 0 and time > 0:
        state = state + states[time - 3]
      if time % 6 == 0 and time > 0:
        state = state + states[time - 6]
      '''
